...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        #Start at the min acceptable attack place
        while range_of_attack != self.min_range and attack_place.entrance is not beehive:
            attack_place = attack_place.entrance
            range_of_attack+=1

        while len(attack_place.bees) == 0 and attack_place.entrance is not beehive and self.min_range<=range_of_attack<self.max_range:
            attack_place = attack_place.entrance
            # print(attack_place)
            range_of_attack+=1

        return random_or_none(attack_place.bees)
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        place.bees = [bee for bee in bees if bee.armor > 0]
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        QueenAnt.num_Queen_bees += 1
        if QueenAnt.num_Queen_bees > 1:
    ...
    def action(self, colony):
        ...
        if self.is_truequeen:
            ...
            action_place = self.place.exit
            while action_place:
                if type(action_place.ant) != QueenAnt:
                    if  action_place.ant and not action_place.ant.is_damage_doubled: 
                        action_place.ant.damage *= 2
                        action_place.ant.is_damage_doubled = True

                    if action_place.ant and action_place.ant.is_container and action_place.ant.contained_ant and not action_place.ant.contained_ant.is_damage_doubled:
                        action_place.ant.contained_ant.damage *= 2
                        action_place.ant.contained_ant.is_damage_doubled = True                            
                action_place = action_place.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.is_truequeen:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(self,colony):
        nonlocal duration
        if duration > 0:
            effect(original_action,bee)(self,colony)
            duration-=1
        else:
            bee.move_forward = True
            original_action(colony)

    bee.action = types.MethodType(new_action,bee)
    ...
    #     def func():
    #         if place != bee.place:
    #             apply_effect(effect,bee,duration-1)
